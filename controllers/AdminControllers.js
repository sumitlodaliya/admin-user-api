...
module.exports.registerdata = (req,res)=> {
    ...
    let existinguser = admin.findOne({email : email});
    if(existinguser){
        return res.status(0).json({"messege" : "user all ready exzist"});
    }
    else{
        admin.create({
            name : req.body.name,
            ...
            if(err)
            {
                ...
                return res.json({"status" : "1","ans" : "data insert success"});
        })
...
module.exports.category = (req,res) => {
    Category.create({
        ...
        registerfaculty_id : req.body.registerfaculty_id
    },(err,data)=>{
        if(err){
            res.json(err);
            return false;
        }
        return res.json({"status" : "1","messege" : "Category successfully add","data" : data});
    })
}


module.exports.categoryviewsubcategory = (req,res) => {
...
//     console.log(email);
//     subcategory.findOne({ student_email: student_email }, (err, userdata) => {
//         if (err) {
//             console.log("Record not found");
//             return false;
//         }
